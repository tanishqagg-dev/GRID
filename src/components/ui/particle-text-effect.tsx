...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    // Generate new color: Using brand deep blacks and grays for high contrast
    const newColor = {
      r: Math.random() > 0.5 ? 0 : 26,
      g: Math.random() > 0.5 ? 0 : 26,
      b: Math.random() > 0.5 ? 0 : 26,
    }
  ...
  const animate = () => {
    ...
    // Background with motion blur - WHITE background
  ...
  return (
    <div className="absolute inset-0 w-full h-full bg-white flex flex-col items-center justify-center">
      ...
      />
      <div className="absolute bottom-24 left-0 right-0 text-neutral-dark/40 text-[10px] text-center pointer-events-none">
        <p className="uppercase font-black tracking-[0.3em] mb-2 opacity-30">Interactive Narrative Protocol</p>
        <p className="font-medium tracking-widest">
          DRAG TO INTERACT • POTENTIAL IS UNIVERSAL
        </p>
      </div>
    </div>
  )
